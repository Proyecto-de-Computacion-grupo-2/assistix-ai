...
const PopUp = ({message, description, nameButton, action}: {
    message: string,
    description: string,
    nameButton: string,
    action?: () => void
    ...
    return (
        <Row className="m-0 p-0 w-100 h-100 personalized-pop-up d-flex justify-content-center align-items-center">
            <Container
                className={`m-0 py-2 px-4 ${description === '' ? 'personalized-pop-up' : 'personalized-width'} bg-light rounded-4 d-flex flex-column align-items-center`}>
                ...
                {description === '' ? (
                    <p className="w-75 text-break text-center py-2 lh-base">
                        Para poder obtener el id del usuario, es necesario que accedas al Mister Fantasy. Una vez dentro
                        debes navegar a la
                        <a href='https://mister.mundodeportivo.com/more' target='_blank'> sección de más</a>.
                        Dentro de esta sección, debes clickear en la información de tu equipo (imagen de equipo,
                        posición, etc.).
                        Una vez dentro, en la URL de la página, encontrarás un número que corresponde al id de tu
                        equipo.
...
export default function LoginPage() {
    ...
    return (
        <Row
            className="m-0 p-0 w-100 vh-100 justify-content-center align-items-center personalized-background personalized-position">
            {message != '' &&
                <PopUp message={message} description={description} nameButton={nameButton} action={handlePopUp}/>}
            <Container
                className="m-0 p-0 w-100 h-100 personalized-blur d-flex justify-content-center align-items-center">
                <Container
                    className="m-0 py-4 w-25 bg-light rounded-4 personalized-width d-flex flex-column align-items-center">
                    ...
                    <Container
                        className="flex-grow- m-0 w-75 h-100 d-flex flex-column align-items-center justify-content-between personalized-position">
                        ...
                        <form className="w-100 h-auto d-flex flex-column justify-content-around"
                              onSubmit={isLogin ? handleLogin : handleRegister}>
                            {
                                ...
                                <>
                                    ...
                                    <input required type='text' className="form-control mb-2"
                                           placeholder="Ingrese su id de usuario" value={id_user}
                                           onChange={(e) => setUserId(e.target.value)}/>
                                </>
                            }
                            <label className="form-label user-select-none">Correo electrónico</label>
                            <input required type="email" className="form-control mb-2" placeholder="Ingrese su correo"
                                   value={email} onChange={(e) => setEmail(e.target.value)}/>
                            <label className="form-label user-select-none">Contraseña</label>
                            <input required type="password" className="form-control mb-4"
                                   placeholder="Ingrese su contraseña" value={password}
                                   onChange={(e) => setPassword(e.target.value)}/>
                            <button type='submit'
                                    className="btn btn-primary user-select-none mb-4">{isLogin ? 'Iniciar sesión' : 'Registrarse'}</button>
                        </form>
                        <p className="personalized-text user-select-none"
                           onClick={handleFormStatus}>{isLogin ? '¿No tienes cuenta?' : '¿Ya tienes cuenta?'}
                            <span>{isLogin ? 'Registrate aquí' : 'Inicia sesión aquí'}</span></p>
                    </Container>
                </Container>
            </Container>
        </Row>
